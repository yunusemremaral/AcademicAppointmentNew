...
import RandevuKart from '../../components/Ogrenci/Randevular/RandevuKart';
import EmptyState from '../../components/Ogrenci/Randevular/RandevuYok';
...
const Randevular: React.FC = () => {
  ...
  // Randevu iptali işlemi - useCallback ile optimize edildi
  const handleDeleteRandevu = useCallback(async (id: number) => {
    if (window.confirm('Bu randevuyu iptal etmek istediğinizden emin misiniz?')) {
      try {
        setIsLoading(true);
        // API isteği gönder - yeni cancelAppointment fonksiyonunu kullan
  ...
  return (
    <div className="p-2 sm:p-3 md:p-5 max-w-full font-poppins">
      {/* Sayfa başlığı */}
      <div className={`p-3 sm:p-4 ${themeColors.primary.main} rounded-xl shadow-md mb-4 sm:mb-5 backdrop-blur-sm bg-opacity-95`}>
        <div className="flex items-center">
          <Dock size={16} className="text-white mr-2.5" />
          <h1 className="text-base sm:text-lg font-semibold text-white">Randevularım</h1>
        </div>
        <p className="text-white/90 text-xs mt-1.5 max-w-2xl">
          Akademisyenlerle olan randevularınızı görüntüleyin ve yönetin.
        </p>
      </div>
              ...
              <div className="space-y-3 mt-4">
                {gelecekRandevular.map(randevu => (
                  <RandevuKart
                    key={randevu.id}
                    randevu={randevu}
                    onDeleteClick={handleDeleteRandevu}
                  />
                ))}
              </div>
            </div>
